...
export default function PlayerModal({ onDone }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Get or generate a persistent UUID for this browser — completely invisible to the user
      ...
      const res  = await fetch("/api/players", {
        ...
        body: JSON.stringify({ name: trimmed, uuid }),
      });
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="modal-title" style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: "0.68rem", fontWeight: 600, color: "var(--text-3)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "1rem" }}>
          Welcome to Spelldle
        </p>
        <h2 id="modal-title" style={{ fontSize: "1.4rem", color: "var(--text)", marginBottom: "0.35rem", fontWeight: 600, letterSpacing: "-0.02em" }}>
          What's your name?
        </h2>
        ...
        <label htmlFor="player-name" style={labelStyle}>Display name</label>
        <input
        ...
        />
        {error && <p role="alert" style={{ color: "var(--error)", fontSize: "0.78rem", marginTop: "5px" }}>{error}</p>}

        <button onClick={handleSubmit} disabled={loading} aria-busy={loading}
          style={{ width: "100%", marginTop: "1.25rem", padding: "12px", background: "var(--primary-container)", border: "none", borderRadius: "var(--radius-md)", color: "white", fontFamily: "'DM Sans', sans-serif", fontSize: "0.9rem", fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", opacity: loading ? 0.6 : 1, cursor: "pointer", transition: "opacity 0.15s" }}>
          {loading
